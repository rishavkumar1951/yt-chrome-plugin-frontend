...
//   const sentimentDiv = document.getElementById("sentiment");
  ...
  const API_URL = 'http://192.168.14.28:5000';
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (match && match[1]) {
      ...
      const comments_with_timestamp_and_author = await fetchComments_with_timestamp_and_author(videoId);
    ...
    //   const predictions = 0
      ...
      if (predictions) {
        ...
        const predictions_with_timestamps = await getSentiment_predict_with_timestamps(comments_with_timestamp_and_author);
        const sentimentCounts = { "1": 0, "0": 0, "-1": 0 };
        const sentimentData = []; // For trend graph
        const totalSentimentScore = predictions_with_timestamps.reduce((sum, item) => sum + parseInt(item.sentiment), 0);
        predictions_with_timestamps.forEach((item, index) => {
        ...
        const uniqueCommenters = new Set(comments_with_timestamp_and_author.map(comment => comment.authorId)).size;
        const totalWords = comments_with_timestamp_and_author.reduce((sum, comment) => sum + comment.text.split(/\s+/).filter(word => word.length > 0).length, 0);
...
function displayComments(sentimentData) {
  ...
  async function fetchComments_with_timestamp_and_author(videoId) {
    let comments = [];
    let pageToken = null; // Initialize as null
    try {
        do {
  ...
  async function getSentiment_predict_with_timestamps(comments) {
    try {
...
document.getElementById("sendRequest").addEventListener("click", async () => {
    ...
    try {
        const response = await fetch("http://192.168.14.28:5000/predict", {
            method: "POST",
            headers: {
